export default {
  ...
  auth: {
    ...
    affiliateCodeInvalid: '返利邀请码无效',
    affiliateCodeNotFound: '返利邀请码不存在',
    affiliateCodeDisabled: '返利功能暂未开启',
    affiliateCodeValidating: '返利邀请码正在验证中，请稍候',
    affiliateCodeInvalidCannotRegister: '返利邀请码无效，请检查后重试或清空该字段',
  ...
  admin: {
    ...
    riskControl: {
      ...
      requestRecords: '日常用户请求记录',
      requestRecordsHint: '展示 API Key 用户手动发起请求时记录的输入摘要，不展示 AI 返回内容；图片内容不会展示；仅在请求记录开关开启后记录新请求。',
      emptyRequestRecords: '暂无请求记录。请确认请求已打到当前后端；历史请求正文不会回填。',
      hiddenImageRequest: '图片请求（不展示图片内容）',
      extractKeywords: '提取关键词',
      extractingKeywords: '提取中',
      keywordExtractionTitle: '提取审计关键词',
      keywordExtractionModel: '提取模型',
      keywordExtractionModelHint: '默认优先使用内容审计配置中的系统侧模型和 API Key；如该模型仅支持 Moderations，可在此改为可生成文本的模型。',
      keywordExtractionSource: '请求内容',
      keywordExtractionCandidates: '候选关键词',
      keywordExtractionCandidateHint: '点击提取后会在这里显示候选关键词。',
      keywordExtractionCandidateMeta: '已提取 {count} 个候选词，模型：{model}',
      keywordExtractionEmpty: '暂无候选关键词',
      keywordExtractionFailed: '关键词提取失败',
      clearExtractedKeywords: '清空候选',
      confirmAddKeywords: '确认加入关键词',
      keywordExtractionAdded: '已加入 {count} 个关键词',
      keywordExtractionAddFailed: '加入关键词失败',
      keywordExtractionAllExists: '候选关键词已全部存在',
      saveConfig: '保存内容审计配置',
      ...
      requestRecordsEnabled: '记录日常用户请求',
      requestRecordsEnabledHint: '开启后才会保存 API Key 手动请求中的输入摘要，用于下方表格展示；数据量可能较大，默认关闭。',
      ...
      keywordBanDurationMinutes: '关键词命中封禁时长（分钟）',
      keywordBanDurationHint: '命中关键词后，该用户后续请求在此时长内直接返回 403；设为 0 表示只拦截当次请求。',
      ...
      filters: {
        search: '按用户/Key/摘要搜索',
        requestSearch: '按分组/账号/Key/请求内容搜索',
        from: '开始时间',
        to: '结束时间',
        allGroups: '全部分组',
        allEndpoints: '全部端点',
    ...
    accounts: {
      ...
      dataImportJsonPlaceholder: '可粘贴完整导出 JSON，或直接粘贴账号数组。示例：',
    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          customUsers: {
            title: '专属用户配置',
            description: '为指定用户设置专属返利邀请码或专属返利比例。仅展示已设置过专属配置的用户。',
            addButton: '添加专属用户',
          ...
          modal: {
            ...
            errorEmpty: '至少填写一项：专属返利邀请码或专属返利比例',
          },
          batchModal: {
