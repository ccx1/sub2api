export default {
    accountImportSettings: {
      ...
      description: 'Fill missing settings for newly imported accounts. Explicit values in the import take priority. These defaults do not change existing accounts during repeated imports.',
      enabled: 'Enable shared import defaults',
      ...
      regionPreserveHint: 'When unspecified, imported and existing account region rules apply. A known subscription country can still constrain the proxy region automatically.',
      regionFallbackHint: 'Billing data is preferred for region matching. The default country below is used when the account subscription country and currency cannot identify a region.',
      regionFallbackCountry: 'Default country when billing is unknown',
      regionFallbackCountryOptional: 'Unspecified (routing is rejected if billing is unknown)',
      ticketConfigured: 'Set a default ticket proxy',
      ticketOverride: 'Override the ticket proxy for this import',
      ticketInheritHint: 'When unspecified, new accounts use import defaults. If import defaults are disabled, the global policy applies.',
      proxyInheritHint: 'When no proxy is selected, new accounts use import defaults. Explicitly selecting “No proxy” overrides this with a direct connection.',
      ticketHint: 'When unspecified, imported and existing ticket proxy settings apply. This only affects supported OpenAI accounts.',
      fixedProxyRequired: 'Select an available fixed proxy.',
      loadFailed: 'Failed to load import defaults. Please retry.',
      proxiesLoadFailed: 'Failed to load proxies. Retry before saving these settings.',
      saved: 'Import defaults saved',
      saveFailed: 'Failed to save import defaults'
    },
    scheduledTests: {
    ...
    proxies: {
      ...
      ipStatusButton: 'IP Status',
      ipStatusTitle: 'Cooled / Disabled IPs',
      ipStatusDescription: 'View egress IPs that are cooling down or permanently disabled after repeated ticket failures.',
      ipStatusFilter: 'IP status type',
      ipStatusAll: 'All types',
      ipStatusCooling: 'Cooling down',
      ipStatusDisabled: 'Disabled',
      ipStatusCount: '{count} IPs total',
      ipStatusEmpty: 'No matching IPs',
      ipStatusLoadFailed: 'Failed to load IP status. Please retry.',
      ipStatusColumns: {
        ip: 'IP address',
        status: 'Status',
        until: 'Cooling until',
        rounds: 'Failed rounds',
      ...
      batchLocation: 'Batch edit location',
      batchLocationTitle: 'Batch edit proxy location',
      batchLocationHint: 'Set a manual country / region for {count} selected proxies. Choose unspecified to clear it.',
      batchLocationSaved: 'Updated location for {count} proxies',
      batchLocationFailed: 'Failed to edit proxy locations',
      countryOptional: 'Unspecified (auto-detect)',
      countryHint: 'Use this region when detection cannot determine a country, including for IPv6 proxies. A successfully detected country takes priority.',
      dataImportCountryOptional: 'Keep regions from the file',
      dataImportCountryHint: 'A selection applies to every proxy in this import. Leave unselected to keep the file values. The manual region is used when detection returns no country.',
      batchCountryOptional: 'Unspecified (detect after import)',
      batchCountryHint: 'Apply this region to all newly added proxies as a fallback when detection returns no country, including for IPv6. A successfully detected country takes priority.',
