...
export default {
    accounts: {
      ...
      randomProxyHint: 'Initially select randomly among eligible proxies with low occupancy. Each account keeps reusing its healthy proxy without scheduled rotation. Select again and rebuild related connections when configuration changes or the proxy becomes unavailable.',
      randomProxyMaxReuseMinutes: 'Proxy rotation interval (minutes)',
      randomProxyMaxReuseHint: 'Switch after 3 consecutive proxy-related failures within a 5-minute window. A successful request clears the consecutive failure count. Account protection can be enabled with reusable random proxies.',
      ...
      randomProxyBalanceHint: 'Shares the per-proxy account limit in system settings with ticket harvesting. Counts fixed assignments and current random associations, once per account per proxy. Healthy proxies remain in use. Full proxies receive no new accounts; if none are available, use the policy below.',
      ...
      randomProxyRegionFallback: 'Region proxy fallback',
      ...
      codexTicketCredential: {
        title: 'Ticket credential mode',
        inherit: 'Use global settings',
        state: 'STATE only (legacy)',
        cookie_state: 'Cookie + STATE (experimental)',
        cookie: 'Cookie only (experimental)',
        inheritHint: 'Follow the global ticket mode by default. An override affects only this account. Returning to global settings clears the account override.',
        stateHint: 'The legacy STATE flow no longer indicates reliable target model recovery.',
        experimentalHint: 'Cookie modes are experimental and do not guarantee model recovery. When credentials are unavailable, the global “Pause model scheduling without a valid ticket” setting determines whether business requests are blocked.',
        ttl_seconds: 'Default Cookie revalidation interval (seconds)',
        refresh_before_seconds: 'Cookie revalidation ahead (seconds)',
        useGlobal: 'Empty to use global settings',
        timingHint: 'Empty values inherit global Cookie settings; 0 disables early revalidation. Credentials with a known expiry remain usable while due for revalidation. Cookies without an expiry use this interval as a fallback.',
        modeInvalid: 'Choose a valid ticket credential mode.',
        ttlInvalid: 'Default Cookie revalidation interval must be an integer from 1 to 3600, or empty to inherit.',
        refreshInvalid: 'Cookie revalidation ahead must be an integer from 0 to 3599, or empty to inherit.',
        refreshOrder: 'Cookie revalidation ahead must be shorter than the default revalidation interval.'
      },
      proxyRegion: {
        title: 'Proxy region matching',
        off: 'Any region',
        billing: 'Match billing region',
        manual: 'Choose region manually',
        country: 'Target country / region',
        chooseCountry: 'Choose a country / region',
        countryRequired: 'Choose a valid country / region for proxy matching.',
        scopeHint: 'Applies to account forwarding and all ticket proxy modes. Only proxies with a detected country can match. Global fixed ticket URLs are rejected when their region cannot be verified.',
        sourceCurrency: '{country}, inferred from {currency} billing',
        sourceCountry: '{country}, from subscription price country',
        sourceFallback: '{country}, from the default region when billing cannot be identified',
        sourceManual: '{country}, manually selected',
        billingPending: 'After saving, match each account using its subscription details: JPY maps to Japan and PHP to the Philippines. USD, EUR, or unknown currencies cannot determine a region; routing will be rejected until a region is set manually.',
        unknownBilling: 'Currency {currency} cannot determine a region, and no subscription price country is available. Routing will be rejected; refresh the subscription details or choose a region manually.',
        matchingCount: '{count} proxies in the current list match this region. Pool scope, availability, and capacity still apply.',
        fixedMismatch: 'The selected fixed proxy has a different or unknown country. Your selection is preserved; choose another proxy or test its egress region before use.',
        directFallback: 'Direct fallback is enabled: if no proxy matches, the account can connect directly and its egress region cannot be guaranteed.',
        directBlocked: 'No proxy is selected. Ordinary direct connections are rejected when region matching is enabled; choose a fixed proxy or a random pool.',
        ticketHint: 'Ticket proxies follow the same account region requirement, including when inheriting global settings.',
        bulkApply: 'Change proxy region matching'
      },
      codexTicketProxy: {
        strategy: 'Initial harvest proxy selection',
        affinity: 'Reuse healthy proxy',
        roundRobin: 'Choose randomly from the pool',
        strategyHint: 'Applies only to the initial pool selection. The selected proxy is reused until consecutive failures reach the threshold in ticket settings, then another proxy is chosen randomly. Success resets the failure count; silence, cooldowns and account budgets still limit requests.',
        title: 'Ticket proxy',
        account: 'Use account outbound proxy',
        inherit: 'Use global policy',
        random: 'Random proxy',
        fixed: 'Fixed proxy',
        accountHint: 'Reuse the account outbound settings, including its fixed proxy, random scope, and proxy group. No separate selection is needed. Ticket requests also connect directly when the account does.',
        inheritHint: 'Follow the global ticket proxy policy, including subsequent changes.',
        randomHint: 'Harvest from the selected proxy pool. Reuse or replace the chosen proxy according to the consecutive failure threshold in ticket settings. This setting is independent of the business verification proxy.',
        fixedHint: 'Only use the selected proxy to harvest tickets. Stop when it is unavailable; never switch proxies or connect directly.',
        listUnavailable: 'Proxy unavailable',
        required: 'Select an available fixed ticket proxy.',
        unavailable: 'The saved ticket proxy was deleted or is unavailable. Its ID is preserved; select another proxy to continue.',
        bulkApply: 'Update ticket proxy',
        bulkUnsupported: 'Bulk ticket proxy settings require OpenAI OAuth / Setup Token accounts. Filter by platform and account type first. Shadow accounts inherit their parent.'
      },
      ...
      dataImportJsonText: 'JSON data',
      dataImportJsonPlaceholder: 'Paste exported account JSON here',
      ...
      codexTicketUnsupported: 'This account does not participate in Codex ticket harvesting',
      failedToToggleCodexTicket: 'Failed to toggle ticket harvesting',
      codexTicketHistory: {
        outcome: 'Classified outcome',
        unsuccessful: 'Not issued',
        summaryCompatibility: 'Legacy not-issued totals include deferrals, cancellations and policy rejections, and are not an upstream failure rate. Old records are not assigned invented classifications.',
        classificationSince: 'Classified totals have been recorded since {time}.',
        title: 'Ticket history',
        total: 'Total attempts',
        success: 'Successful',
        failed: 'Failed',
        retainedHint: 'Counts track harvest attempts since recording began, including business verification when enabled. Success requires acceptance under the verification settings at that time and saving. Only the latest {limit} completed attempts are retained.',
        ...
        exchangeRetainedHint: 'Request/response details are retained only for the latest {limit} new attempts. Older attempts keep model summaries; past raw payloads cannot be recovered.',
        requestedModel: 'Requested model',
        harvestModel: 'Harvest response model',
        businessModel: 'Verification response model',
        reportedModels: 'Reported response models',
        modelNotRecorded: 'Not recorded / stage not started',
        modelNotReported: 'Response did not report a model',
        responseNotReceived: 'No response received',
        modelsTruncated: 'Model list truncated',
        viewExchange: 'View messages',
        hideExchange: 'Hide messages',
        exchangeTitle: 'Request and response messages',
        exchangeRedactedHint: 'Displayed and copied messages are redacted. Authentication data, cookies and complete tickets are hidden. Bodies may be omitted when over the retention limit, incompletely read or unsafe to parse.',
        exchangeRawHint: 'Raw messages; up to 64 KiB per body and 16 KiB of headers are retained.',
        rawRequest: 'Raw request',
        rawResponse: 'Raw response',
        redactedRequest: 'Redacted request (older record)',
        redactedResponse: 'Redacted response (older record)',
        copyRawExchange: 'Copy raw message',
        rawExchangeCopied: 'Raw message copied',
        exchangeUnavailable: 'Messages were not retained: past payloads cannot be recovered, older details have been cleared, or this stage did not start.',
        harvestStage: 'Harvest stage',
        businessStage: 'Business verification stage',
        requestMessage: 'Request',
        responseMessage: 'Response',
        requestNotRecorded: 'Request not recorded',
        responseNotRecorded: 'Not recorded / no response received',
        headers: 'Headers',
        body: 'Body',
        bodyBytes: 'Original body: {bytes} bytes',
        bodyOmitted: 'Body not fully retained or omitted',
        headersTruncated: 'Headers truncated',
        emptyHeaders: 'No headers recorded',
        emptyBody: 'Empty body',
        copyExchange: 'Copy redacted message',
        exchangeCopied: 'Redacted message copied',
        exchangeCopyFailed: 'Copy failed. Select and copy the message manually.',
        harvestLength: 'Harvest length',
        businessLength: 'Business verification length',
        lengthNotObserved: 'Not recorded / no response received',
        ticketMissing: 'No ticket returned (0)',
        attemptRule: 'Rule for this attempt',
        businessVerification: 'Quality probe rounds passed',
        businessVerificationModels: 'Models reported during probes',
        autoRule: 'Dynamic validation (no fixed length)',
        strictRule: 'Strict matching; required length {length}',
        strictRuleWithoutLength: 'Strict matching; target length not recorded',
        ruleNotRecorded: 'Not recorded',
        attemptRejectedLengths: 'Rejected lengths for this attempt: {lengths}',
        harvestProxy: 'Harvest proxy',
        businessProxy: 'Business verification proxy',
        direct: 'Direct connection',
        notSelected: 'Not selected (stage not started)',
        duration: 'Duration: {ms} ms',
        empty: 'No ticket attempts recorded yet',
        filteredEmpty: 'No ticket attempts match the selected filters',
        filters: {
          all: 'All',
          reason: 'Failure / invalidation reason',
          startedFrom: 'Started from (local time)',
          startedTo: 'Started through (local time)',
          apply: 'Apply filters',
          reset: 'Clear filters',
          invalidRange: 'Enter valid dates with the start no later than the end',
          hint: 'Filters apply to all retained records. Cumulative counts above do not change. Issued means accepted under the verification settings at that time and saved; it does not guarantee continued validity.',
        ...
        lifecycle: {
          ...
          expiresAt: 'Cache expires at',
          source: 'Triggered by',
          reportedModels: 'Trigger response models',
          returnedLength: 'Trigger response ticket length',
          unknownHint: 'Older records did not retain invalidation details; they cannot be recovered.',
          statuses: {
            not_issued: 'Not issued',
            issued: 'Issued',
            invalidated: 'Invalidated',
            ttl_elapsed: 'Cache expired',
            unknown: 'Not recorded'
          },
          reasons: {
            response_model_mismatch: 'Business response model mismatch',
            response_ticket_rejected: 'Response ticket rejected by strict rules',
            cookie_changed: 'Cookie updated, deleted, or given a shorter lifetime',
            ttl_expired: 'Local cache TTL expired',
            unknown: 'Not recorded'
          },
          sources: {
            http: 'HTTP business response',
            websocket: 'WebSocket business response',
            websocket_handshake: 'WebSocket handshake',
            websocket_prewarm: 'WebSocket prewarm',
            unknown: 'Not recorded'
          }
        },
        loadFailed: 'Failed to load ticket history. Please retry.',
        retry: 'Retry',
        retryEligible: 'Re-harvest (force once)',
        retrying: 'Re-harvesting…',
        retryScheduled: 'Started re-harvest for {count} model(s): {models}',
        retrySkipped: 'No configured Codex ticket model is available',
        retryFailed: 'Re-harvest request failed. Please try again later.',
        reasons: {
          verified: 'Verified and saved',
          verification_skipped: 'Harvested and saved (business egress not verified)',
          harvest_failed: 'Harvest failed',
          harvest_http_rejected: 'Harvest request rejected',
          harvest_transport_failed: 'Harvest connection failed',
          ticket_not_matched: 'No eligible ticket harvested',
          ticket_missing: 'No ticket returned',
          ticket_length_mismatch: 'Ticket length mismatch',
          ticket_format_invalid: 'Invalid ticket format',
          ticket_length_rejected: 'Ticket length matched a rejected-length rule',
          harvest_model_mismatch: 'Harvest response model mismatch',
          business_model_mismatch: 'Business verification response model mismatch',
          harvest_response_incomplete: 'Harvest response did not complete',
          business_response_incomplete: 'Business verification response did not complete',
          harvest_response_failed: 'Harvest model response failed',
          business_response_failed: 'Business verification model response failed',
          controls_changed: 'Account or ticket settings changed during harvesting',
          business_proxy_unavailable: 'Business verification proxy unavailable',
          business_failed: 'Business verification failed',
          business_http_rejected: 'Business verification request rejected',
          business_transport_failed: 'Business verification connection failed',
          business_ticket_rejected: 'Ticket rejected during business verification',
          publish_failed: 'Failed to save ticket'
        }
      },
      columns: {
        name: 'Name',
      ...
      openai: {
        ...
        codexTicketPool: 'Ticket pool',
        codexTicketQualityPassed: 'Model quality check: passed',
        codexTicketQualityFailed: 'Model quality check: failed',
        codexTicketQualityChecking: 'Model quality check: running',
        codexTicketQualityPending: 'Model quality check: pending confirmation',
        codexTicketQualitySkipped: 'Model quality check: skipped',
        codexTicketQualityPaused: 'Model paused',
        codexTicketQualityReasonPrefix: 'Reason: ',
        codexTicketQualityReasons: {
          not_checked: 'Not checked yet',
          checking: 'Checking in background',
          scheduled: 'Queued for background check',
          insufficient_ttl: 'Ticket lifetime too short to check now',
          cooldown: 'Waiting for the next allowed check',
          disabled: 'Background quality check disabled',
          no_ticket: 'No usable ticket available',
          stale: 'Ticket or check context changed',
          egress_changed: 'Egress changed; waiting for new result',
          proxy_unavailable: 'Check egress unavailable',
          account_unavailable: 'Account currently unavailable for checks',
          model_not_configured: 'Model not in the ticket configuration',
          shared_state_unavailable: 'Shared check state unavailable',
          quality_paused: 'Consecutive low-quality tickets reached the threshold; harvesting and checks paused',
          capacity: 'Check concurrency is full',
          account_capacity: 'Account check concurrency is full',
          capability_passed: 'Capability quick check passed',
          capability_failed: 'Capability quick check did not pass',
          canary_passed: 'Canary question passed',
          canary_failed: 'Canary question failed',
          model_mismatch: 'Declared model does not match the target',
          fingerprint_match: 'Fingerprint matches the target',
          fingerprint_mismatch: 'Fingerprint leans toward another candidate',
          fingerprint_uncertain: 'Fingerprint evidence is insufficient',
          network_error: 'Check network request failed',
          upstream_auth_error: 'Upstream credential check failed',
          upstream_rate_limited: 'Upstream rate limited',
          upstream_error: 'Upstream request error',
          response_incomplete: 'Upstream response incomplete',
          timeout: 'Check timed out',
          quarantine_persist_failed: 'Quarantine not persisted; will retry'
        },
        codexTicketPoolRouteAffinityAvailable: 'Route affinity: available',
        codexTicketPoolRouteAffinityUnavailable: 'Route affinity: unavailable',
        codexTicketPoolRouteAffinityUnknown: 'Route affinity: not yet checked',
        codexTicketPoolRouteAffinityConnections: 'Current connections: {count}',
        codexTicketPoolRouteExpires: 'Route expires {time}',
        codexTicketPoolRouteAffinityHint: 'Shows same-route connection availability in this process. Availability does not mean the model quality check has passed.',
        codexTicketPoolHint: 'Tickets are stored separately per model and are reusable. Counts are not remaining request quotas.',
        codexTicketPoolAvailable: '{count}/{capacity} available',
        codexTicketPoolAboveCapacity: '{count} available · Target capacity {capacity}',
        codexTicketPoolCount: '{count} available',
        codexTicketPoolReserve: '{count} in reserve',
        codexTicketPoolExpiring: '{count} expiring soon',
        codexTicketPoolNextExpiry: 'Next expiry {time}',
        codexTicketPoolEmpty: 'No available tickets',
        codexTicketPoolPaused: 'Paused without tickets',
        codexTicketPoolCredentialAvailable: 'Credentials ready',
        codexTicketPoolCredentialRevalidation: 'Revalidation due · Usable',
        codexTicketPoolCredentialExpired: 'Credentials expired',
        codexTicketPoolCredentialRevoked: 'Credentials revoked',
        codexTicketPoolCredentialMissing: 'No credentials',
        codexTicketPoolRevalidationHint: 'Revalidation is due. Current credentials remain usable until their actual expiry; new Cookies replace them only after verification.',
        codexTicketPoolRevalidateAt: 'Revalidate at: {time}',
        codexTicketPrimaryReady: 'Primary {time}',
        codexTicketUsingStandbyCompact: 'Standby {time}',
        codexTicketPrimaryMissing: 'No primary',
        codexTicketPrimaryRevoked: 'Primary revoked',
        ...
        codexTurnTicketPrimary: 'Primary',
        codexTurnTicketStandbyReady: 'Standby ready',
        codexTurnTicketUsingStandby: 'Using standby',
        codexTurnTicketStandbyExpires: 'Standby expires: {time}',
