...
export default {
    accounts: {
      randomProxyRegionFallback: '地区代理回退',
      randomProxyRegionFallbackHint: '优先选择与账号地区匹配的代理；地区没有可用代理时，可回退到当前随机代理池。',
      randomProxyRegionFallbackOptions: { none: '仅使用账号地区代理', pool: '地区无可用时回退到当前代理池' },
      ...codexTicketTools,
      codexModelQualityRecord: '模型质量检测记录',
      codexModelQualityMenu: '质量记录',
      codexTicketCredential: {
        title: '打票凭据模式',
        inherit: '跟随全局',
        state: '仅 STATE（旧流程）',
        cookie_state: 'Cookie＋STATE（实验）',
        cookie: '仅 Cookie（实验）',
        inheritHint: '默认跟随打票设置中的全局模式。选择独立模式仅影响当前账号；切回“跟随全局”会清除账号覆盖。',
        stateHint: '旧 STATE 流程已不能作为稳定恢复目标模型的依据。',
        experimentalHint: 'Cookie 模式仍在验证中，不保证恢复目标模型；缺少有效凭据时，是否暂停业务请求由全局“没有合格票据时暂停模型调度”开关决定。',
        ttl_seconds: 'Cookie 默认复验间隔（秒）',
        refresh_before_seconds: 'Cookie 提前复验时间（秒）',
        useGlobal: '留空跟随全局',
        timingHint: '留空继承全局 Cookie 参数，填 0 表示不提前复验。已有实际期限时按间隔复验，仍有效的票继续使用；Cookie 未提供期限时以此间隔兜底。',
        modeInvalid: '请选择有效的打票凭据模式。',
        ttlInvalid: 'Cookie 默认复验间隔须为 1–3600 的整数，或留空跟随全局。',
        refreshInvalid: 'Cookie 提前复验时间须为 0–3599 的整数，或留空跟随全局。',
        refreshOrder: 'Cookie 提前复验时间必须小于默认复验间隔。'
      },
      proxyRegion: {
        title: '代理地区匹配',
        off: '不限制地区',
        ...
        countryRequired: '请为地区匹配选择有效的国家 / 地区。',
        scopeHint: '同时约束账号转发和所有打票代理模式；仅匹配已检测国家的代理。无法确认地区的全局固定打票地址会被拒绝。',
        sourceCurrency: '{country}，根据 {currency} 计费推断',
        sourceCountry: '{country}，来源：订阅定价国家',
        sourceFallback: '{country}，来源：账单地区无法识别时的默认地区',
        sourceManual: '{country}，来源：手动设置',
        billingPending: '保存后按每个账号查询到的订阅资料匹配；JPY 对应日本、PHP 对应菲律宾。USD、EUR 或未知币种无法确定地区时会拒绝选路，请改为手动指定。',
        unknownBilling: '当前币种 {currency} 无法确定地区，且没有订阅定价国家。请求会被拒绝选路，请刷新账号订阅资料或手动指定地区。',
        matchingCount: '当前代理列表中有 {count} 条地区匹配；实际可用性还取决于代理池范围、状态和容量。',
        fixedMismatch: '当前固定代理的地区不匹配或尚未检测，已保留原选择；使用前请更换代理或检测其出口地区。',
        directFallback: '已设置“空池时直连”：没有匹配代理时仍会直连，此时无法保证出口地区。',
        directBlocked: '当前账号未选择代理，开启地区匹配后普通直连会被拒绝；请选择固定代理或随机代理池。',
        ticketHint: '打票代理同样受账号地区匹配约束；继承全局不会绕过地区限制。',
        bulkApply: '修改代理地区匹配'
      },
      codexTicketProxy: {
        title: '打票代理',
        ...
        affinity: '优先复用健康代理',
        roundRobin: '从代理池随机选择',
        strategyHint: '仅代理池首次选择时生效。选定后持续复用，连续失败达到打票设置中的阈值后才随机更换，成功会清零失败计数；静默、冷却和账号预算仍限制实际发包。',
        account: '跟随账号出口代理',
        inherit: '跟随全局',
        random: '随机代理',
        fixed: '固定代理',
        accountHint: '直接复用账号出口设置：固定代理、随机范围和代理组均跟随账号，无需重复选择。账号直连时，打票也直连。',
        inheritHint: '使用全局打票代理规则；全局规则调整后，未单独配置的账号自动跟随。',
        randomHint: '在所选代理池中采集，选定后按打票设置的连续失败阈值复用和更换；此设置独立于业务复验出口。',
        fixedHint: '仅使用所选代理打票；代理失效时停止打票，不会自动直连或改用其他代理。',
        listUnavailable: '代理不可用',
        required: '请选择一条可用的固定打票代理。',
        unavailable: '原打票代理已删除或不可用，请重新选择；原选择仍保留。',
        bulkApply: '修改打票代理',
        bulkUnsupported: '仅支持为 OpenAI OAuth / Setup Token 账号批量设置打票代理。请按平台和账号类型筛选后重试；影子账号继承母账号。'
      },
      title: '账号管理',
      ...
      dataImportJsonText: 'JSON 数据',
      dataImportJsonPlaceholder: '在此粘贴导出的账号 JSON',
      ...
      codexTicketEnabled: '已加入打票',
      codexTicketDisabled: '未加入打票',
      codexTicketUnsupported: '该账号不参与 Codex 打票',
      failedToToggleCodexTicket: '切换打票状态失败',
      codexTicketHistory: {
        outcome: '分类结果',
        unsuccessful: '未成功出票',
        summaryCompatibility: '兼容累计中的“未成功出票”包含暂缓、取消和策略拒收，不能作为上游失败率；旧记录不补造分类。',
        classificationSince: '分类累计自 {time} 起记录。',
        title: '打票记录',
        total: '总次数',
        success: '成功',
        failed: '失败',
        retainedHint: '统计为记录启用后的累计采集轮次（含已启用的业务复验）；按当时验证配置验收并保存成功才计为成功。列表仅保留最近 {limit} 条已结束记录。',
        lastAttempt: '最近一次打票',
        startedAt: '开始时间',
        model: '模型',
        result: '结果',
        reason: '失败原因',
        details: '结果详情',
        exchangeRetainedHint: '仅最近 {limit} 条新记录保留请求/响应详情；更早的记录保留模型摘要，旧记录无法补回原始报文。',
        requestedModel: '请求模型',
        harvestModel: '采集实际模型',
        businessModel: '复验实际模型',
        reportedModels: '实际响应模型',
        modelNotRecorded: '未记录/阶段未开始',
        modelNotReported: '响应未报告模型',
        responseNotReceived: '未收到响应',
        modelsTruncated: '模型列表已截断',
        viewExchange: '查看报文',
        hideExchange: '收起报文',
        exchangeTitle: '请求与响应报文',
        exchangeRedactedHint: '展示及复制的报文均已脱敏，认证信息、Cookie 和完整票据不会显示。正文可能因超出保存上限、未完整读取或无法安全解析而被省略。',
        exchangeRawHint: '原始报文；正文最多保存 64 KiB，报文头最多保存 16 KiB。',
        rawRequest: '原始请求',
        rawResponse: '原始响应',
        redactedRequest: '脱敏请求（旧记录）',
        redactedResponse: '脱敏响应（旧记录）',
        copyRawExchange: '复制原始报文',
        rawExchangeCopied: '已复制原始报文',
        exchangeUnavailable: '本条记录未保留报文：旧记录无法补回，超出保留范围的详情已清理，或此阶段未开始。',
        harvestStage: '采集阶段',
        businessStage: '业务复验阶段',
        requestMessage: '请求',
        responseMessage: '响应',
        requestNotRecorded: '未记录请求',
        responseNotRecorded: '未记录/未收到响应',
        headers: '报文头',
        body: '正文',
        bodyBytes: '原始正文：{bytes} 字节',
        bodyOmitted: '正文未完整保存或已省略',
        headersTruncated: '报文头已截断',
        emptyHeaders: '无已记录报文头',
        emptyBody: '正文为空',
        copyExchange: '复制脱敏报文',
        exchangeCopied: '已复制脱敏报文',
        exchangeCopyFailed: '复制失败，请手动选择报文复制',
        harvestLength: '采集长度',
        businessLength: '业务复验长度',
        lengthNotObserved: '未记录/未收到响应',
        ticketMissing: '未返回票据（0）',
        attemptRule: '本次规则',
        businessVerification: '质量探测通过轮次',
        businessVerificationModels: '探测实际返回模型',
        autoRule: '动态验证（不限制固定长度）',
        strictRule: '严格筛选；本次要求 {length}',
        strictRuleWithoutLength: '严格筛选；目标长度未记录',
        ruleNotRecorded: '未记录',
        attemptRejectedLengths: '本次异常长度：{lengths}',
        harvestProxy: '采集代理',
        businessProxy: '业务复验代理',
        direct: '直连',
        notSelected: '未选择（阶段未开始）',
        duration: '耗时 {ms} ms',
        empty: '暂无打票记录',
        filteredEmpty: '没有符合筛选条件的打票记录',
        filters: {
          all: '全部',
          reason: '失败 / 失效原因',
          startedFrom: '开始时间起（本地时间）',
          startedTo: '开始时间止（本地时间）',
          apply: '筛选',
          reset: '清空筛选',
          invalidRange: '请填写有效时间，且开始时间不能晚于结束时间',
          hint: '筛选适用于全部保留记录；顶部累计统计不随筛选变化。已出票仅表示按当时验证配置验收并保存成功，不保证持续有效。',
          attemptReason: '打票：{reason}',
          invalidationReason: '失效：{reason}'
        },
        lifecycle: {
          status: '票据状态',
          reason: '失效原因',
          ...
          expiresAt: '缓存到期时间',
          source: '触发来源',
          reportedModels: '触发响应模型',
          returnedLength: '触发响应票长',
          unknownHint: '旧记录未保存失效信息，无法补回。',
          statuses: {
            not_issued: '未出票',
            issued: '已出票',
            invalidated: '已撤销',
          ...
          reasons: {
            response_model_mismatch: '业务响应模型不匹配',
            response_ticket_rejected: '响应票据未通过严格规则',
            cookie_changed: 'Cookie 已更新、删除或缩短有效期',
            ttl_expired: '本地缓存 TTL 已到期',
            unknown: '未记录'
          },
          sources: {
            http: 'HTTP 业务响应',
            websocket: 'WebSocket 业务响应',
            websocket_handshake: 'WebSocket 握手',
            websocket_prewarm: 'WebSocket 预热',
            unknown: '未记录'
          }
        },
        loadFailed: '加载打票记录失败，请重试',
        retry: '重试',
        retryEligible: '重新打票（强制一次）',
        retrying: '正在重新打票…',
        retryScheduled: '已发起 {count} 个模型的重新打票：{models}',
        retrySkipped: '当前没有可配置的打票模型',
        retryFailed: '重新打票请求失败，请稍后重试',
        reasons: {
          verified: '验证并保存成功',
          verification_skipped: '采集并保存成功（未复核业务出口）',
          harvest_failed: '采集失败',
          harvest_http_rejected: '采集请求被拒绝',
          harvest_transport_failed: '采集连接失败',
          ticket_not_matched: '未采集到符合要求的票据',
          ticket_missing: '未返回票据',
          ticket_length_mismatch: '票据长度不匹配',
          ticket_format_invalid: '票据格式无效',
          ticket_length_rejected: '票据长度命中异常规则',
          harvest_model_mismatch: '采集响应模型不匹配',
          business_model_mismatch: '业务复验响应模型不匹配',
          harvest_response_incomplete: '采集响应未完整结束',
          business_response_incomplete: '业务复验响应未完整结束',
          harvest_response_failed: '采集模型响应失败',
          business_response_failed: '业务复验模型响应失败',
          controls_changed: '采集期间账号或打票配置发生变化',
          business_proxy_unavailable: '业务复验代理不可用',
          business_failed: '业务复验失败',
          business_http_rejected: '业务复验请求被拒绝',
          business_transport_failed: '业务复验连接失败',
          business_ticket_rejected: '票据未通过业务复验',
          publish_failed: '票据保存失败'
        }
      },
      ...
      usageError: '获取失败',
      randomProxy: '随机选择代理',
      randomProxyHint: '首次从符合范围和容量要求的低占用代理中随机选择；同一账号在代理健康时持续复用，不按时间定期更换。配置变化或代理不可用时重新选择并重建相关连接。',
      randomProxyMaxReuseMinutes: '代理更换周期（分钟）',
      randomProxyMaxReuseHint: '代理相关失败在 5 分钟统计窗口内连续达到 3 次后切换；成功请求会清除连续失败计数。账号保护可与稳定复用的随机代理同时开启。',
      randomProxyMaxReuseInvalid: '代理更换周期必须为 0 到 525600 之间的整数分钟。',
      dailyCooldown: {
        title: '每日定时冷却',
        hint: '每天在指定时段暂停接收新请求，到期自动恢复参与调度；不会修改账号的手动禁用、停调或其他限制。',
        bulkApply: '修改所选账号的每日冷却',
        start: '开始冷却',
        end: '恢复调度',
        hourLabel: '{label}（小时）',
        minuteLabel: '{label}（分钟）',
        timezone: '时区',
        timezoneHint: '使用 IANA 时区，例如 Asia/Shanghai（北京时间）、UTC、America/New_York。',
        overnightHint: '结束时间在次日，例如 23:00 开始、次日 08:00 恢复。',
        invalidTime: '请选择有效的开始和结束时间。',
        equalTimes: '开始与结束时间不能相同。',
        invalidTimezone: '请输入有效的 IANA 时区。',
        active: '定时冷却中',
        scheduled: '已设每日冷却',
        schedule: '每天 {start}–{end}（{timezone}）'
      },
      randomProxyBalanceHint: '与打票代理池共用系统设置中的单代理关联上限，统计固定绑定和当前随机关联，同一账号在同一代理上只计一次。健康代理持续复用；达到上限的代理不再接收新账号，全部不可用时按下方策略处理。',
      randomProxyEmptyPoolPolicy: '代理池为空时',
      randomProxyPoolScope: '随机代理范围',
      randomProxyPoolAll: '整个可用代理池',
      randomProxyPoolSelected: '仅指定代理',
      randomProxyPoolChoose: '选择代理（已选 {count} 条）',
      randomProxyPoolSearch: '搜索名称、地址或区域',
      randomProxyPoolNoMatches: '没有匹配的可用代理',
      randomProxyPoolUnavailable: '以下已选代理当前不可用或已删除，保留勾选不会扩大到其他代理：',
      randomProxyPoolUnavailableItem: '代理 #{id}（当前不可用）',
      randomProxyPoolRequired: '请至少选择一个代理，或切换为整个可用代理池。',
      randomProxyPoolSelectedHint: '仅从勾选且有效的代理中按空闲、关联数和质量均衡选择；全部失效或达到关联上限后按下方空池策略处理，不会改用其他代理。',
      randomProxyPoolSelectedCount: '指定 {count} 条代理',
      randomProxyLastUsed: '最近实际出口',
      randomProxyLastDirect: '直连',
      randomProxyNoUsage: '尚无出口记录',
      ...
      randomProxyEmptyPoolPolicyHints: {
        reject: '代理池暂时没有可用代理时拒绝本次请求，不回退直连。',
        disable: '所选范围没有可用代理时拒绝本次请求，并立即禁用账号。恢复代理后需手动启用账号。',
        direct: '代理池暂时没有可用代理时允许本次请求直连。'
      },
      testing: '测试中...',
      ...
      openai: {
        ...
        codexTicketPool: '票据池',
        codexTicketQualityPassed: '模型质量检测：通过',
        codexTicketQualityFailed: '模型质量检测：不通过',
        codexTicketQualityChecking: '模型质量检测：检测中',
        codexTicketQualityPending: '模型质量检测：待确认',
        codexTicketQualitySkipped: '模型质量检测：已跳过',
        codexTicketQualityPaused: '模型已暂停',
        codexTicketQualityReasonPrefix: '原因：',
        codexTicketQualityReasons: {
          not_checked: '尚未检测',
          checking: '正在后台检测',
          scheduled: '已排入后台检测',
          insufficient_ttl: '票据可用寿命不足，暂不检测',
          cooldown: '未到下次检测时间',
          disabled: '未启用后台质量检测',
          no_ticket: '暂无可用票据',
          stale: '票据或检测上下文已变化',
          egress_changed: '出口已变化，等待新出口结果',
          proxy_unavailable: '检测出口不可用',
          account_unavailable: '账号当前不可用于检测',
          model_not_configured: '模型未列入打票配置',
          shared_state_unavailable: '共享检测状态暂不可用',
          quality_paused: '连续低质量票已达阈值，采票与检测已暂停',
          capacity: '检测并发已满',
          account_capacity: '该账号检测并发已满',
          capability_passed: '基础能力快检通过',
          capability_failed: '基础能力快检未达标',
          canary_passed: '金丝雀题通过',
          canary_failed: '金丝雀题未通过',
          model_mismatch: '模型声明与目标不一致',
          fingerprint_match: '数字指纹与目标一致',
          fingerprint_mismatch: '数字指纹偏向其他候选',
          fingerprint_uncertain: '数字指纹证据不足',
          network_error: '检测网络请求失败',
          upstream_auth_error: '上游凭据校验失败',
          upstream_rate_limited: '上游限流',
          upstream_error: '上游请求异常',
          response_incomplete: '上游响应不完整',
          timeout: '检测超时未完成',
          quarantine_persist_failed: '隔离未成功保存，稍后重试'
        },
        codexTicketPoolRouteAffinityAvailable: '路由亲和：可用',
        codexTicketPoolRouteAffinityUnavailable: '路由亲和：不可用',
        codexTicketPoolRouteAffinityUnknown: '路由亲和：待探测',
        codexTicketPoolRouteAffinityConnections: '当前 {count} 条连接',
        codexTicketPoolRouteExpires: '路由到期 {time}',
        codexTicketPoolRouteAffinityHint: '显示当前进程的同路由连接状态；可用不代表模型质量检测通过。',
        codexTicketPoolHint: '按模型独立存票；有效票可重复使用，数量不是剩余调用次数。',
        codexTicketPoolAvailable: '可用 {count}/{capacity} 张',
        codexTicketPoolAboveCapacity: '可用 {count} 张 · 目标容量 {capacity} 张',
        codexTicketPoolCount: '可用 {count} 张',
        codexTicketPoolReserve: '备用 {count} 张',
        codexTicketPoolExpiring: '{count} 张即将到期',
        codexTicketPoolNextExpiry: '最早到期 {time}',
        codexTicketPoolEmpty: '无可用票',
        codexTicketPoolPaused: '无票暂停',
        codexTicketPoolCredentialAvailable: '凭据可用',
        codexTicketPoolCredentialRevalidation: '需复验 · 仍可用',
        codexTicketPoolCredentialExpired: '凭据过期',
        codexTicketPoolCredentialRevoked: '凭据撤销',
        codexTicketPoolCredentialMissing: '暂无凭据',
        codexTicketPoolRevalidationHint: '已到复验时间，现有凭据在实际期限内继续可用；新 Cookie 通过复验后才会替换。',
        codexTicketPoolRevalidateAt: '复验时间：{time}',
        codexTicketPrimaryReady: '主票 {time}',
        codexTicketUsingStandbyCompact: '备用 {time}',
        codexTicketPrimaryMissing: '无主票',
        codexTicketPrimaryRevoked: '主票已撤销',
        ...
        codexTurnTicketPrimary: '主票',
        codexTurnTicketStandbyReady: '备用就绪',
        codexTurnTicketUsingStandby: '正在用备用',
        codexTurnTicketStandbyExpires: '备用票到期：{time}',
