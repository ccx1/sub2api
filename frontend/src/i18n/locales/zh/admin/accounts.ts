export default {
    accounts: {
      ...
      codexTicketUnsupported: '该账号不参与 Codex 292 打票',
      ...
      usageError: '获取失败',
      randomProxy: '随机选择代理',
      randomProxyHint: '同一账号优先复用已选代理，配置变化、代理不可用或达到更换周期时重新选择，并重建相关连接。重新选择时优先关联账号少的代理，同等关联数优先质量更好的代理；连接失败的代理不参与分配，质量降级代理仅在正常代理没有容量时备用。',
      randomProxyMaxReuseMinutes: '代理更换周期（分钟）',
      randomProxyMaxReuseHint: '0 表示不按时间定期更换；正数表示到期后在下一次请求时重新选择。代理仍须符合当前范围、容量和可用性要求。账号保护可与稳定复用的随机代理同时开启。',
      ...
      randomProxyBalanceHint: '与打票代理池共用系统设置中的单代理关联上限，统计固定挂载和最近 10 分钟的动态关联，同一账号在同一代理上只计一次。达到上限的代理不再接收新账号；全部不可用时按下方策略处理。',
