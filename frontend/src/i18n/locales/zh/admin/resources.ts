export default {
    accountImportSettings: {
      title: '导入默认配置',
      description: '为新导入账号补齐未填写的设置。导入数据中明确填写的值优先；重复导入更新已有账号时不应用这些默认值。',
      enabled: '启用统一导入默认配置',
      ...
      regionPreserveHint: '未指定时沿用导入数据与账号原有地区规则；已知订阅国家仍可自动约束代理地区。',
      regionFallbackHint: '选择按账单地区匹配时，优先使用账号订阅中的定价国家或币种；无法识别时使用下方默认国家。',
      regionFallbackCountry: '账单无法识别时的默认国家 / 地区',
      regionFallbackCountryOptional: '不指定（识别失败时仍拒绝选路）',
      ticketConfigured: '设置默认打票代理',
      ticketOverride: '单独指定本次打票代理',
      ticketInheritHint: '未单独指定时，新账号使用导入默认配置；未启用导入配置时沿用全局策略。',
      proxyInheritHint: '未选择代理时，新账号使用导入默认配置；主动选择“不使用代理”可覆盖为直连。',
      ticketHint: '未指定时沿用导入数据与原有打票代理设置。该项仅对支持打票的 OpenAI 账号生效。',
      fixedProxyRequired: '请选择一个有效的固定代理。',
      loadFailed: '加载导入默认配置失败，请重试。',
      proxiesLoadFailed: '加载代理列表失败，暂时无法保存配置，请重试。',
      saved: '导入默认配置已保存',
      saveFailed: '保存导入默认配置失败'
    },
    scheduledTests: {
      title: '定时测试',
      addPlan: '添加计划',
    ...
    proxies: {
      ...
      ipStatusButton: 'IP 状态',
      ipStatusTitle: '已冷却 / 已禁用 IP',
      ipStatusDescription: '查看因连续打票失败而暂时冷却或永久禁用的出口 IP。',
      ipStatusFilter: 'IP 状态类型',
      ipStatusAll: '全部类型',
      ipStatusCooling: '冷却中',
      ipStatusDisabled: '已禁用',
      ipStatusCount: '共 {count} 个 IP',
      ipStatusEmpty: '暂无符合条件的 IP',
      ipStatusLoadFailed: '加载 IP 状态失败，请重试。',
      ipStatusColumns: {
        ip: 'IP 地址',
        status: '状态',
        until: '冷却截止',
        rounds: '失败轮次',
        failedAccounts: '失败账号数',
        lastFailure: '最近失败'
      },
      createProxy: '添加代理',
      editProxy: '编辑代理',
      ...
      batchLocation: '批量编辑位置',
      batchLocationTitle: '批量编辑代理位置',
      batchLocationHint: '为选中的 {count} 个代理设置手动国家 / 地区；选择不指定可清除手动位置。',
      batchLocationSaved: '已更新 {count} 个代理的位置',
      batchLocationFailed: '批量编辑位置失败',
      countryOptional: '不指定（自动检测）',
      countryHint: 'IPv6 等代理未能检测出国家时，使用这里指定的地区；检测成功并返回国家时，以检测结果为准。',
      dataImportCountryOptional: '保留文件中的地区',
      dataImportCountryHint: '选择后统一应用于本次导入的所有代理；未选择则保留文件原地区。检测未返回国家时使用手动地区。',
      batchCountryOptional: '不指定（导入后可检测）',
      batchCountryHint: '统一设置本次新增代理的地区，用于 IPv6 等代理未检测出国家时的兜底；检测成功并返回国家时，以检测结果为准。',
      backupProxyRequired: '请选择备用代理',
      batchInput: '代理列表',
      batchInputFormat: '冒号分隔格式',
      batchFormatAuto: '自动识别（歧义行不导入）',
      batchFormatHostFirst: '主机:端口:用户名:密码',
      batchFormatCredentialsFirst: '用户名:密码:主机:端口',
      batchAmbiguousHint: "{count} 行存在格式歧义，未计入有效代理。请明确选择冒号分隔格式，或改用带 {'@'} 的格式。",
      batchDefaultProtocol: '无协议头时使用',
      batchDefaultProtocolHint: '不带协议头的代理将按此选项自动补全。',
    ...
    spendGuard: {
      title: '异常消耗冻结',
      description: '按 Token 速率和异常错误率自动冻结异常 API Key，复核后可手动解冻。',
      settings: '保护规则',
      enableAuto: '启用自动冻结',
      windowMinutes: '统计窗口（分钟）',
      tokensPerMinute: 'Token/分钟阈值',
      minRequests: '最少请求数',
      maxErrorRate: '错误率阈值（0-1）',
      intervalSeconds: '检查间隔（秒）',
      settingsHint: '请求数不足时不会冻结；自动冻结的 Key 会保持禁用，直到管理员手动解冻。',
      key: 'API Key',
      requests: '请求数',
      tokensPerMin: 'Token/分钟',
      errRate: '错误率',
      status: '状态',
      frozen: '已冻结',
      statusActive: '启用',
      ...
      statusExpired: '已过期',
      unfreeze: '解冻',
      unfreezeSuccess: 'API Key 已解冻',
      unfreezeFailed: '解冻 API Key 失败',
      events: '冻结事件',
      noData: '当前窗口暂无 API Key 活跃记录。',
      loadFailed: '加载异常消耗冻结数据失败',
      saveFailed: '保存保护规则失败',
      saveSuccess: '保护规则已保存',
      reasonTokenRate: 'Token 消耗速率超过阈值',
      reasonErrorRate: '错误率超过阈值',
      reasonManual: '手动解冻'
    },
