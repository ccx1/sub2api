...
describe('UseKeyModal', () => {
  ...
  it('renders iCode provider, GPT-5.5 and goals feature in Codex config', () => {
    const wrapper = mount(UseKeyModal, {
      props: {
        show: true,
        apiKey: 'sk-test',
    ...
    const configToml = codeBlocks.find((content) => content.includes('model_provider = "iCode"'))

    expect(configToml).toBeDefined()
    expect(configToml).toContain('[model_providers.iCode]')
    expect(configToml).toContain('name = "iCode"')
  ...
  it('renders iCode provider, GPT-5.5 and goals feature in Codex WebSocket config', async () => {
    const wrapper = mount(UseKeyModal, {
      props: {
        show: true,
        apiKey: 'sk-test',
    ...
    expect(configToml).toBeDefined()
    expect(configToml).toContain('model_provider = "iCode"')
    expect(configToml).toContain('[model_providers.iCode]')
    expect(configToml).toContain('name = "iCode"')
    expect(configToml).toContain('model = "gpt-5.5"')
    expect(configToml).toContain('review_model = "gpt-5.5"')
