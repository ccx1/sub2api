    ...
    <template v-if="overview">
      <div v-if="overview.tiers.length" class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        <article v-for="tier in overview.tiers" :key="`${tier.platform}-${tier.tier}`" class="card min-w-0 p-4" data-test="tier-card">
          ...
          <dl class="mt-4 grid gap-3 border-t border-gray-200 pt-3 dark:border-dark-700">
            <div class="min-w-0">
              ...
              <dd class="mt-1 break-all text-xl font-semibold tabular-nums" data-test="participating-accounts">{{ stale ? '—' : count(tier.participating_accounts) }} / {{ stale ? '—' : count(tier.available_accounts) }} / {{ count(tier.total_accounts) }}</dd>
            </div>
...
function tierState(tier: SharedPoolTier): keyof typeof statusKeys {
  if (props.stale || count(tier.participating_accounts) === '—') return 'unknown'
  return tier.participating_accounts > 0 ? 'available' : 'unavailable'
}
