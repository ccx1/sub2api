...
const tier = (overrides: Partial<SharedPoolTier> = {}): SharedPoolTier => ({
  platform: 'openai', tier: 'free', total_accounts: 10, available_accounts: 9, schedulable_accounts: 7, available: true,
  participating_accounts: 7, participating_concurrency: 36, participating_concurrency_unlimited: false,
...
const overview = (overrides: Partial<SharedPoolOverview> = {}): SharedPoolOverview => ({
  total_accounts: 10, available_accounts: 9, schedulable_accounts: 7, concurrency_capacity: 30, concurrency_unlimited: false,
  participating_accounts: 7, participating_concurrency: 36, participating_concurrency_unlimited: false,
...
describe('shared account resource overview', () => {
  it('shows one card per platform and tier without the three aggregate cards', () => {
    const wrapper = render(overview({ tiers: [tier(), tier({ tier: 'pro', total_accounts: 3, available_accounts: 2, participating_accounts: 0, schedulable_accounts: 0, available: false })] }))
    ...
    expect(wrapper.get('[data-test="participating-accounts"]').text()).toBe('7 / 9 / 10')
  ...
  it('retains unavailable tiers with their full account counts and distinguishes platform and account types', () => {
    const wrapper = render(overview({ tiers: [
      tier({ tier: 'free', participating_accounts: 0, available_accounts: 2, schedulable_accounts: 0, available: false }),
    ...
    expect(cards[0].get('[data-test="participating-accounts"]').text()).toBe('0 / 2 / 10')
  ...
  it.each([undefined, -1, NaN, Infinity])('does not label invalid participating count %s as available', value => {
    const wrapper = render(overview({ tiers: [tier({ participating_accounts: value as number })] }))
    expect(wrapper.get('[data-test="participating-accounts"]').text()).toBe('— / 9 / 10')
    expect(wrapper.get('[data-test="tier-status"]').text()).toBe('sharedPool.overviewStatusUnknown')
  })

  it.each([undefined, -1, NaN, Infinity])('keeps unknown available count %s distinct from zero', value => {
    const wrapper = render(overview({ tiers: [tier({ available_accounts: value as number })] }))
  ...
  it('keeps large counts readable and statistic explanations keyboard accessible', () => {
    const wrapper = render(overview({ tiers: [tier({ total_accounts: 987654321, available_accounts: 234567890, participating_accounts: 123456789 })] }))
    const value = wrapper.get('[data-test="participating-accounts"]')
    expect(value.text()).toBe('123456789 / 234567890 / 987654321')
