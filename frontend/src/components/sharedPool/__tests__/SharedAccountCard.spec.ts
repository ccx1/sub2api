...
describe('shared account card controls', () => {
  ...
  it('keeps legacy enable separate from explicit dispatch authorization', async () => {
    const wrapper = render()
    ...
    await wrapper.get('[role="switch"][aria-label="sharedPool.legacySharing"]').trigger('click')
    expect(wrapper.emitted('enable')).toEqual([[false]])
  ...
  it('shows a compact cooldown schedule without overriding a manually disabled state', async () => {
    ...
    expect(wrapper.get('[role="switch"][aria-label="sharedPool.sharing"]').attributes('aria-checked')).toBe('false')
    expect(wrapper.emitted('enable')).toBeUndefined()
    wrapper.unmount()
  })
