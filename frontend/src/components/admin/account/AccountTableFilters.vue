<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    <Select :model-value="filters.type" class="w-40" :options="tOpts" :aria-label="t('admin.accounts.accountType')" @update:model-value="updateType" @change="$emit('change')" />
    <Select :model-value="filters.shared" class="w-40" :options="sharedOpts" :aria-label="t('sharedPool.accounts')" @update:model-value="updateShared" @change="$emit('change')" />
...
const updateShared = (value: string | number | boolean | null) => { emit('update:filters', { ...props.filters, shared: value }) }
...
const sharedOpts = computed(() => [
  { value: 'all', label: t('sharedPool.allAccounts') },
  { value: 'shared', label: t('sharedPool.sharedOnly') },
  { value: 'platform', label: t('sharedPool.platformOnly') }
])
