...
function render(extra = {}) { return mount(Dashboard, { props: { accounts: [], account, manualRecord: manual, ...extra }, global: { stubs: { teleport: true } } }) }
...
describe('Pelican record dashboard', () => {
  it('renders outside an isolated app container and restores focus after closing', async () => {
    const host = document.createElement('div')
    host.style.isolation = 'isolate'
    host.setAttribute('role', 'dialog')
    host.innerHTML = '<button>Open history</button>'
    document.body.append(host)
    const opener = host.querySelector('button')!
    opener.focus()
    const wrapper = mount(Dashboard, {
      attachTo: host,
      props: { accounts: [], account, manualRecord: manual, onClose: () => wrapper.unmount() }
    })
    try {
      await flushPromises()
      const overlay = document.body.querySelector<HTMLElement>('[data-testid="pelican-dashboard"]')!
      expect(overlay.parentElement).toBe(document.body)
      expect(host.contains(overlay)).toBe(false)
      expect(host.hasAttribute('inert')).toBe(true)
      expect(document.activeElement).toBe(overlay)
      overlay.querySelector<HTMLButtonElement>('article button')!.click()
      await flushPromises()
      const detail = overlay.querySelector<HTMLElement>('[data-testid="record-detail"]')!
      expect(document.activeElement).toBe(detail)
      detail.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }))
      await flushPromises()
      expect(overlay.querySelector('[data-testid="record-detail"]')).toBeNull()
      expect(document.activeElement).toBe(overlay.querySelector('article button'))
      overlay.querySelector<HTMLButtonElement>('[data-testid="close-pelican-dashboard"]')!.click()
      await flushPromises()
