<template>
  <Teleport to="body">
  <div ref="dashboard" class="fixed inset-0 z-[60] overflow-y-auto overscroll-contain bg-[var(--app-bg)] p-4 text-[var(--text)] outline-none sm:p-6" role="dialog" aria-modal="true" tabindex="-1" data-testid="pelican-dashboard" :aria-label="t('admin.accounts.pelicanTest.history')" @keydown="handleKeydown">
    <div class="mx-auto min-w-0 max-w-[1800px]" :inert="selected ? true : undefined" :aria-hidden="selected ? 'true' : undefined">
      <header class="mb-4 flex flex-col items-start justify-between gap-4 sm:flex-row">
        ...
        <div class="flex shrink-0 gap-2"><button type="button" class="btn btn-secondary" :disabled="refreshing" @click="refresh">{{ t('common.refresh') }}</button>
        <button type="button" class="btn btn-secondary" data-testid="close-pelican-dashboard" @click="emit('close')">{{ t('common.close') }}</button></div>
      </header>
      ...
      <div v-else-if="cards.length" class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
        <article v-for="card in visibleCards" :key="card.key" class="relative min-w-0 overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--surface)] shadow-sm transition hover:border-primary-300 hover:shadow-md" data-testid="pelican-record-card">
          <div class="p-4">
            <h3 class="font-semibold [overflow-wrap:anywhere]">{{ card.account.name }}</h3>
            ...
            <div class="mt-2 space-y-1 text-xs text-gray-500 [overflow-wrap:anywhere]">
            ...
            <div class="mt-3 aspect-[4/3] overflow-hidden rounded-xl bg-gray-50">
              ...
              <p v-else class="p-4 text-sm text-red-500 [overflow-wrap:anywhere]">{{ card.loadError || card.record.error || t('admin.accounts.pelicanTest.invalidHtml') }}</p>
            </div>
          ...
          <button type="button" class="absolute inset-0 z-10 rounded-lg focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary-500" :aria-label="`${card.account.name} · ${t('admin.accounts.pelicanTest.preview')}`" @click="openRecord(card, $event)" />
        </article>
      </div>
    ...
    <div v-if="selected" ref="detail" class="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4 outline-none" data-testid="record-detail" role="dialog" aria-modal="true" :aria-label="selected.account.name" tabindex="-1" @click.self="closeRecord">
      <div class="flex h-full min-w-0 w-full max-w-6xl flex-col overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--surface)]">
        <header class="flex shrink-0 items-start justify-between gap-3 border-b px-4 py-3">
          ...
          <button type="button" class="btn btn-secondary shrink-0" @click="closeRecord">{{ t('common.close') }}</button>
        </header>
        ...
        <pre v-else class="overflow-auto whitespace-pre-wrap p-4 text-sm [overflow-wrap:anywhere]">{{ selected.loadError || selected.record.error || selected.record.output }}</pre>
      </div>
    </div>
  </div>
  </Teleport>
...
async function openRecord(card: Card, event?: MouseEvent) {
  detailOpener = event?.currentTarget as HTMLElement | null ?? document.activeElement as HTMLElement | null
  selected.value = card
  await nextTick()
...
async function closeRecord() {
  selected.value = null
  await nextTick()
  if (alive) (detailOpener?.isConnected ? detailOpener : dashboard.value)?.focus({ preventScroll: true })
}
function handleKeydown(event: KeyboardEvent) {
  if (event.key === 'Escape') {
    event.preventDefault()
    event.stopImmediatePropagation()
    if (selected.value) void closeRecord()
    else emit('close')
    return
  }
  if (event.key !== 'Tab') return
  const panel = detail.value || dashboard.value
  const elements = Array.from(panel?.querySelectorAll<HTMLElement>('button:not([disabled]), iframe:not([tabindex="-1"]), [href], [tabindex]:not([tabindex="-1"])') ?? [])
  const first = elements[0]
  const last = elements[elements.length - 1]
  const active = document.activeElement
  if (!first || active === panel || !panel?.contains(active) || (event.shiftKey ? active === first : active === last)) {
    event.preventDefault()
...
onMounted(() => {
  ...
  previousAriaHidden = lowerDialog?.getAttribute('aria-hidden') ?? null
  previousInert = lowerDialog?.hasAttribute('inert') ?? false
  dashboard.value?.focus({ preventScroll: true })
...
onBeforeUnmount(() => {
  ...
  // 父级测试弹窗继续持有滚动锁，子窗只恢复焦点。
  if (dashboard.value?.contains(document.activeElement) && returnFocus?.isConnected) returnFocus.focus({ preventScroll: true })
})
</script>
