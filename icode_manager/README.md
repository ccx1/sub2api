...
- 档次红包：管理员配置多个档次，每个档次包含 `档次标识`、`VIP 名称`、`门槛金额`、`门槛依据`、`使用记录天数`、`动画组合` 和独立兑换码池。每个档次的兑换码在对应档次卡片里单独导入；用户领取时会按所选门槛依据判断金额，并按需校验近 N 天内是否有使用记录，匹配最高且有库存的可领取档次后从该档次码池随机发放一个兑换码。
...
1. `历史总充值`：使用用户历史总充值 `total_recharged`，并核减命中本地红包码池的历史兑换码金额。
2. `最近充值`：使用 `NOW() - N 天` 到当前时间内的 `balance` 和 `admin_balance` 兑换记录，并核减同一窗口内命中本地红包码池的兑换码金额；选择该依据时 `使用记录天数` 必须大于 0。
3. `当前余额`：使用用户当前 `balance` 字段，不再额外按本地红包码池核减。
4. 核减后金额小于 0 时按 0 处理。
