...
- 档次红包：管理员配置多个档次，每个档次包含 `档次标识`、`VIP 名称`、`门槛金额`、`使用记录天数`、`动画组合` 和独立兑换码池。每个档次的兑换码在对应档次卡片里单独导入；用户领取时会根据 sub2api 充值记录计算有效充值金额，并按需校验近 N 天内是否有使用记录，匹配最高可领取档次后从该档次码池随机发放一个兑换码。

档次红包的门槛金额和使用记录天数只用于服务端资格判断，不会返回给用户领取页展示。`使用记录天数` 留空或填 `0` 表示不限制；填 `7` 表示该档次要求用户近 7 天内在 sub2api `usage_logs` 中有使用记录。`成功提示模板` 仅在档次红包中显示和保存，普通红包不配置该模板。档次红包领取成功文案由管理台的 `成功提示模板` 配置，默认值为：
...
1. 未配置 `使用记录天数` 的档次，优先使用 `balance-history` 返回的历史 `total_recharged`。
2. 配置了 `使用记录天数 > 0` 的档次，门槛金额只统计 `NOW() - N 天` 到当前时间内的 `balance` 和 `admin_balance` 记录，不计入更早的历史充值。
3. 如果用户在 sub2api 里兑换过的兑换码命中本地所有红包码池，则按对应统计范围内的记录 `value` 从资格金额中核减。
4. 核减后金额小于 0 时按 0 处理。
5. 若档次配置了 `使用记录天数 > 0`，还会查询 sub2api 数据库 `usage_logs`，按使用记录落库时间判断，要求 `usage_logs.created_at >= NOW() - N 天` 至少存在一条记录。
