...
用户后续说“拉取新代码”“更新上游”或“拉取最新代码并合并”时，默认同时拉取并合并以下两个上游，不能只处理其中一个；用户明确指定单一来源时，以当次要求为准：
...
3. 按 `old-origin/main` → `ranxi/production` 的顺序合并本次获取的提交；已经包含的来源记录为已核验、无需重复合并。优先保留本地 UI 和功能定制，遇到前端冲突时不能简单使用 `theirs` 覆盖本地 UI。
...
5. 对两个来源本次获取的 SHA 分别执行 `git merge-base --is-ancestor <SHA> HEAD`，确认均已进入最终提交历史，并运行定向验证；没有验证证据时，不要声称“合并完成”“可部署”。
6. 两个来源合并完成后，针对最终整合结果统一执行前端生产构建，再用最新前端产物构建后端 Linux amd64 可执行二进制；同次出包的本地主版本第四位只递增一次，不因两个来源或构建重试重复递增。最终回复必须给出两个上游的合并 SHA，以及二进制路径、版本、最终提交号、大小和 SHA256。
