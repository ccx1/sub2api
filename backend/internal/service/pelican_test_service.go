...
func createClaudeTestPayload(ctx context.Context, modelID string) (map[string]any, error) {
	if options, ok := pelicanTestOptionsFromContext(ctx); ok {
		return createPelicanClaudePayload(modelID, options.prompt)
	}
	return createTestPayload(modelID)
}

func createOpenAIResponsesTestPayload(ctx context.Context, modelID string, isOAuth bool) map[string]any {
	if options, ok := pelicanTestOptionsFromContext(ctx); ok {
		return createPelicanOpenAIPayload(modelID, isOAuth, options.prompt, options.reasoningEffort)
	}
