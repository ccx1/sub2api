...
func (s *OpenAIGatewayService) latestOpenAITurnAccountForGroup(
	...
	latest = carryOpenAITurnRuntimeProxy(selected, latest)
...
// 随机代理的出口只在运行时分配、不落库；权威快照里读不到它。仍为随机代理模式时
// 沿用本次调度已分配（并已由随机代理校验链路验证）的出口，避免准入把出口清空。
// 固定代理冷却切换同理：仅当库中固定代理仍是切换前的来源时沿用运行时替代出口。
func carryOpenAITurnRuntimeProxy(selected, latest *Account) *Account {
	if selected == nil || latest == nil || latest == selected {
		return latest
	}
	if selected.IsRandomProxy() && latest.IsRandomProxy() {
		copied := *latest
		copied.ProxyID = selected.ProxyID
		copied.Proxy = selected.Proxy
		return &copied
	}
	origin := selected.fixedProxyOrigin
	if origin == nil || selected.IsRandomProxy() || latest.IsRandomProxy() ||
		latest.ProxyID == nil || *latest.ProxyID != origin.ID ||
		(latest.Proxy != nil && latest.Proxy.URL() != origin.URL()) {
		return latest
	}
	copied := *latest
	configured := *origin
	if latest.Proxy != nil {
		configured = *latest.Proxy
	}
	copied.ProxyID, copied.Proxy, copied.fixedProxyOrigin = selected.ProxyID, selected.Proxy, &configured
	return &copied
}
...
// A connection is bound to the account route fingerprint seen at handshake.
// 票据快照的有效性由 openAICodexTicketWSReceipt（连接池 Acquire 时 validate）负责，
// 这里只拦截握手之后账号路由/身份发生变化或连接超龄的复用。
...
func (s *OpenAIGatewayService) bindOpenAIWSHandshake(account *Account, model string, _ http.Header) *openAIWSTurnBinding {
	return &openAIWSTurnBinding{model: strings.TrimSpace(model), fingerprint: openAITurnRouteFingerprint(account), createdAt: time.Now()}
}

func (s *OpenAIGatewayService) checkOpenAIWSBinding(account *Account, _ string, b *openAIWSTurnBinding) error {
