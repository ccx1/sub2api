		...
		for _, change := range []string{"ticket_expiry", "account_expiry", "ticketless_model"} {
			t.Run(mode+"/"+change, func(t *testing.T) {
				...
				a.Extra[openAICodexTicketExtraKey(firstModel)] = &openAICodexTicket{
					...
					CapturedAt: time.Now(), IssuedAt: time.Now(), ExpiresAt: ticketExpiry, Verified: true,
					AccountBinding: openAICodexTicketAccountBinding(a), Egress: openAICodexTicketEgress(resolveAccountProxyURL(a)),
				}
