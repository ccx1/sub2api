...
// 第三个参数为兼容保留字段，当前已不再作为注册准入校验使用。
// affiliateCode 用于邀请返利绑定，仅在新用户注册时使用。
func (s *AuthService) LoginOrRegisterOAuthWithTokenPair(ctx context.Context, email, username, _ string, affiliateCode string) (*TokenPair, *User, error) {
