...
func TestMoveExcelBPSOn403RollsBackFailures(t *testing.T) {
	for _, failureAt := range []string{"delete", "insert", "account", "outbox", "commit"} {
		t.Run(failureAt, func(t *testing.T) {
			...
			mock.ExpectBegin()
			mock.ExpectQuery("(?s)SELECT extra FROM accounts.*FOR UPDATE").WithArgs(int64(27), "{\"access_token\":\"test-token\"}", "7").
				WillReturnRows(sqlmock.NewRows([]string{"extra"}).AddRow(`{}`))
			mock.ExpectQuery("(?s)SELECT platform, status, subscription_type, is_exclusive, is_shared_pool, require_oauth_only FROM groups.*FOR SHARE").WithArgs(int64(7)).
				WillReturnRows(sqlmock.NewRows([]string{"platform", "status", "subscription_type", "is_exclusive", "is_shared_pool", "require_oauth_only"}).
					AddRow(service.PlatformOpenAI, service.StatusActive, service.SubscriptionTypeStandard, false, false, false))
