	...
	for _, key := range []string{"codex_credits_snapshot", "codex_referral_snapshot"} {
		for _, tc := range []struct {
			...
			t.Run(key+"/"+tc.name, func(t *testing.T) {
				...
				require.NoError(t, err)
				mock.ExpectExec(`(?s)UPDATE accounts SET extra = .*`+regexp.QuoteMeta("COALESCE(extra, '{}'::jsonb) || $1::jsonb")+`.*`+regexp.QuoteMeta(", updated_at = NOW() WHERE id = $2 AND deleted_at IS NULL")).
