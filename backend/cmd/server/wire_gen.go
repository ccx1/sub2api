...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	sharedPoolRepository := repository.NewSharedPoolRepository(client, db, schedulerCache)
	proxyLatencyCache := repository.NewProxyLatencyCache(redisClient)
	settingRepository := repository.NewSettingRepository(client)
	groupRepository := repository.NewGroupRepositoryWithProxyHealth(client, db, proxyLatencyCache)
	proxyRepository := repository.NewProxyRepositoryWithProxyPool(client, db, redisClient)
	settingService := service.ProvideSettingService(settingRepository, groupRepository, proxyRepository, configConfig)
	proxyPoolAllocator := repository.NewProxyPoolAllocator(client, redisClient, proxyLatencyCache, settingService)
	accountRepository := repository.NewAccountRepositoryWithProxyPool(client, db, schedulerCache, proxyPoolAllocator)
	userRepository := repository.NewUserRepository(client, db)
	adminGroupRepository := repository.NewAdminGroupRepositoryWithProxyHealth(client, db, proxyLatencyCache)
	adminAccountRepository := repository.NewAdminAccountRepositoryWithProxyPool(client, db, schedulerCache, proxyPoolAllocator)
	...
	antiDegradeService := service.ProvideAntiDegradeService(adminService, configConfig, pluginManager, settingRepository, accountRepository)
	sharedPoolEarningsRepository := repository.NewSharedPoolEarningsRepository(db)
	sharedPoolService := service.NewSharedPoolService(sharedPoolRepository, accountRepository, groupRepository, adminService, antiDegradeService, sharedPoolEarningsRepository, compositeTokenCacheInvalidator)
	...
	sharedPoolOAuthHandler := handler.NewSharedPoolOAuthHandler(sharedPoolService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService)
	...
	sharedPoolAutoTransferRepository := repository.NewSharedPoolAutoTransferRepository(db)
	sharedPoolAutoTransferService := service.ProvideSharedPoolAutoTransferService(sharedPoolAutoTransferRepository, apiKeyAuthCacheInvalidator, billingCacheService, leaderLockCache, db)
	sharedPoolHandler := handler.ProvideSharedPoolHandler(sharedPoolService, sharedPoolEarningsRepository, apiKeyService, billingCacheService, accountTestService, sharedPoolOAuthHandler, accountUsageService, openAIQuotaService, rateLimitService, configConfig, settingService, sharedPoolAutoTransferService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, accountOpsHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, securityPolicyHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, spendGuardHandler, antiDegradeHandler, upstreamBillingProbeService, ollamaCloudUsageService, openAIGatewayService, proxyPoolAllocator, settingService, openCodeGoUsageService)
