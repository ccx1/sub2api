...
## 管理员：Docker 部署（可选，替代上面的 systemd 步骤）

Sub2API 使用 `deploy/docker-compose.yml`（或 `docker-compose.local.yml`）部署时，在 `deploy` 目录执行：

```bash
F="-f docker-compose.yml -f docker-compose.copilot.yml"
docker compose $F build copilot-sidecar
# 一次性 GitHub 设备登录：按提示在浏览器打开网址并输入设备码
docker compose $F run --rm copilot-sidecar python /app/login.py --state-dir /data
docker compose $F up -d copilot-sidecar
# 读取 sidecar-key，填入后台账号的 API Key
docker compose $F run --rm copilot-sidecar cat /data/sidecar-key
```

后台账号 Base URL 填 `http://copilot-sidecar:18765/v1`，其余同下表。sidecar 不对宿主机开放端口；容器需能访问 GitHub/Copilot，宿主机需代理时在 compose 中设置 `HTTPS_PROXY`。重新登录后执行 `docker compose $F restart copilot-sidecar`。Sub2API 以宿主机二进制运行时，只用 `-f docker-compose.copilot.yml`，取消注释 `127.0.0.1:18765:18765` 端口映射，Base URL 填 `http://127.0.0.1:18765/v1`。
...
| Base URL | 同主机二进制部署填 `http://127.0.0.1:18765/v1`；Docker 部署填 `http://copilot-sidecar:18765/v1`；不要留空，也不要填 GitHub 地址 |
